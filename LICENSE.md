# wiig License

wiig is available under dual licensing:

## 1. Open Source License: GPL-3.0-or-later

wiig is distributed under the **GNU General Public License v3.0 or later (GPL-3.0-or-later)**.

This license allows you to:
- Use wiig freely
- Study and modify the source code
- Distribute original or modified versions

For the full text of GPL-3.0, see the file `COPYING` or visit:
https://www.gnu.org/licenses/gpl-3.0.html

### GPL-3.0-or-later Conditions Summary:
- Include a copy of the GPL-3.0 license with your distribution
- Document any modifications to wiig
- Derivative works must also be distributed under GPL-3.0-or-later
- Provide access to the complete corresponding source code

---

## 2. Commercial License

If you wish to use wiig under a **proprietary/commercial license** (e.g., to avoid GPL obligations or for closed-source distribution), please contact:

**inquiry@cleverplant.com**

### Commercial License Inquiries:
- **License Type**: Proprietary/Commercial
- **Use Cases**:
  - Closed-source commercial applications
  - Proprietary extensions
  - Custom development agreements
  - OEM licensing
- **Support**: Custom support and maintenance agreements available

---

## 3. How to Choose

| Scenario | License |
|----------|---------|
| Open-source project (GPL-compatible) | GPL-3.0-or-later |
| Proprietary/closed-source application | Commercial License required |
| Distribution requiring proprietary terms | Commercial License required |
| Custom/enterprise agreement | Commercial License (contact us) |

---

## 4. Attribution

When using wiig under GPL-3.0-or-later, please include this notice:

```
wiig - Datalog (.dl) formatter and syntax highlighter
Copyright (C) CleverPlant
Licensed under GPL-3.0-or-later
```

---

## 5. Source Code Header

Each source file should include a single-line SPDX identifier:

```
/* SPDX-License-Identifier: GPL-3.0-or-later */
/* Copyright (C) CleverPlant. Commercial: inquiry@cleverplant.com */
```

The full GPL header text is kept once at the repository root in this file
to avoid per-file boilerplate.

---

## 6. Legal Disclaimer

wiig is provided "AS IS" without any warranty. See GPL-3.0 for full disclaimer.

For questions about licensing:
- **Open Source (GPL-3.0-or-later)**: Refer to https://www.gnu.org/licenses/gpl-3.0.html
- **Commercial Licensing**: inquiry@cleverplant.com

---

**Last Updated**: 2026-05-05
